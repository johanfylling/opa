...
func (q *Query) PartialRun(ctx context.Context) (partials []ast.Body, support []*ast.Module, err error) {
	...
	err = e.Run(func(e *eval) error {
		...
		body = unrollTemplateStringCalls(body)
...
func unrollTemplateStringCalls(body ast.Body) ast.Body {
	// TODO: Can we drop the calls collection and do the call type check fully in unrollTemplateStringCall()?
	var calls []*ast.Expr
	ast.WalkExprs(body, func(expr *ast.Expr) bool {
		if expr.IsCall() && expr.Operator().Equal(ast.InternalTemplateString.Ref()) {
			calls = append(calls, expr)
		} else if t, ok := expr.Terms.(*ast.Term); ok {
			if c, ok := t.Value.(ast.Call); ok && c.Operator().Equal(ast.InternalTemplateString.Ref()) {
				calls = append(calls, expr)
			}
		}
		return true
	...
	for _, call := range calls {
		if ls, ok := unrollTemplateStringCall(call, body); ok {
			body = dropBodyLines(ls, body)
		} else {
			fmt.Printf("failed to unroll %v\n", call)
			// Unroll changes
			return cpy
		}
	}
...
func unrollTemplateStringCall(expr *ast.Expr, body ast.Body) ([]int, bool) {
	var operands []*ast.Term

	if expr.IsCall() {
		operands = expr.Operands()
	} else if t, ok := expr.Terms.(*ast.Term); ok {
		if c, ok := t.Value.(ast.Call); ok {
			operands = c.Operands()
		}
	}
	...
	origParts, ok := operands[0].Value.(*ast.Array)
	if !ok {
		return nil, false
	}

	parts := make([]ast.Node, 0, origParts.Len())

	var linesToDrop []int

	err := origParts.Iter(func(term *ast.Term) error {
		switch v := term.Value.(type) {
		case ast.Null, ast.Boolean, ast.Number, ast.String:
			// Primitives are added as-is
			parts = append(parts, term)
		case ast.Set:
			if t, ok := unrollSet(v); ok {
				parts = append(parts, ast.NewExpr(t))
			} else {
				return fmt.Errorf("failed to unroll single-value set")
			}
		case ast.Var:
			// Vars are unrolled if they reference a set-comprehension or single-value sets, otherwise we abort
			if t, ls, ok := unrollTemplateExpression(v, body); ok {
				linesToDrop = append(linesToDrop, ls...)
				parts = append(parts, ast.NewExpr(t))
			} else {
				return fmt.Errorf("failed to unroll template-expression: %v", v)
			}
		default:
			return fmt.Errorf("unsupported template-expression type: %T", v)
		}
	...
	if len(operands) == 2 {
		expr.Terms = ast.Equality.Expr(operands[1], ast.TemplateStringTerm(true, parts...)).Terms
	} else {
		expr.Terms = ast.TemplateStringTerm(true, parts...)
	}

	return linesToDrop, true
}

func unrollSet(s ast.Set) (*ast.Term, bool) {
	// Single-value sets are unwrapped
	if s.Len() != 1 {
		return nil, false
	}
...
func unrollTemplateExpression(v ast.Var, b ast.Body) (*ast.Term, []int, bool) {
	// Find var assignment in body, we only expect compiler rewrites to be first-level body expressions.
	for i := range b {
		if b[i].IsAssignment() || b[i].IsEquality() {
			// We expect the var assignment to be generated by the compiler, so we only need to check the LHS for name equality
			if v.Equal(b[i].Operand(0).Value) {
				candidate := b[i].Operand(1)

				switch c := candidate.Value.(type) {
				case ast.Set:
					t, changed := unrollSet(c)
					if changed {
						return t, []int{i}, true
					}
				case *ast.SetComprehension:
					t, changed := unrollSetComprehension(c)
					if changed {
						return t, []int{i}, true
					}
				default:
...
func unrollSetComprehension(c *ast.SetComprehension) (*ast.Term, bool) {
	v, ok := c.Term.Value.(ast.Var)
	if !ok {
		return nil, false
	}

	t, linesToDrop := unrollVar(v, c.Body)
	//sort.Ints(linesToDrop)
	//// FIXME: if this was a set, we could just assert the number of lines is equal to the body size
	//for i := len(linesToDrop) - 1; i >= 0; i-- {
	//	c.Body = dropSliceIndex(c.Body, linesToDrop[i])
	//}

	c.Body = dropBodyLines(linesToDrop, c.Body)
...
func dropBodyLines(ls []int, b ast.Body) ast.Body {
	sort.Ints(ls)
	// FIXME: if this was a set, we could just assert the number of lines is equal to the body size
	for i := len(ls) - 1; i >= 0; i-- {
		b = dropSliceIndex(b, ls[i])
	}

	return b
}

func unrollVar(v ast.Var, b ast.Body) (*ast.Term, []int) {
	// Find var assignment in body, we only expect compiler rewrites to be first-level body expressions.
	for i := range b {
		if b[i].IsAssignment() || b[i].IsEquality() {
			// We expect the var assignment to be generated by the compiler, so we only need to check the LHS for name equality
			if v.Equal(b[i].Operand(0).Value) {
				...
				t := b[i].Operand(1)
				t, ls = unrollTerm(t, b)
				return t, append(ls, i)
			}
		} else if b[i].IsCall() {
			terms, ok := b[i].Terms.([]*ast.Term)
			if ok && v.Equal(terms[len(terms)-1].Value) {
				linesToDrop := []int{i}
				terms = terms[:len(terms)-1]
				for _, t := range terms[1:] {
					ut, ls := unrollTerm(t, b)
					*t = *ut
					linesToDrop = append(linesToDrop, ls...)
				}
				// FIXME: We get screwed up here if unrollTerm() above dropped lines in b above i, as that screws up the location of i
				// TODO: return which lines to drop instead?
				return ast.CallTerm(terms...), linesToDrop
			}
		}
	}
...
func dropSliceIndex[T any](s []T, i int) []T {
	l := len(s)
	if l == 0 || i < 0 || i >= l {
		return s
	}
...
func unrollTerm(t *ast.Term, b ast.Body) (*ast.Term, []int) {
	var linesToDrop []int

	ast.WalkTerms(t, func(candidate *ast.Term) bool {
		if v, ok := candidate.Value.(ast.Var); ok {
			unrolled, ls := unrollVar(v, b)
			if unrolled != nil {
				candidate.Value = unrolled.Value
				linesToDrop = append(linesToDrop, ls...)
			}
		}
		return false

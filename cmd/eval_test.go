...
func TestFeaturesCapabilities(t *testing.T) {
	tests := []struct {
		note          string
		policy        string
		features      []string
		...
		{
			note: "string-prefix refs used, no feature",
			policy: `package test
a.b.c := 1`,
			expectedError: "rule heads with refs are not supported: a.b.c",
		},
		{
			note: "string-prefix refs used, with feature",
			policy: `package test
a.b.c := 1`,
			features: []string{"rule_head_ref_string_prefixes"},
		},
		{
			note: "string-prefix refs not used, with feature",
			policy: `package test
a := 1`,
			features: []string{"rule_head_ref_string_prefixes"},
		},
		// TODO: enable these when the rego.v1 import capability feature is added
		//		{
		//			note: "rego.v1 imported, no feature",
		//			policy: `package test
		//import rego.v1`,
		//			expectedError: "Invalid import, `rego.v1` is not supported by current capabilities",
		//		},
		//		{
		//			note: "rego.v1 imported, with feature",
		//			policy: `package test
		//import rego.v1`,
		//			features: []string{"rego_v1_import"},
		//		},
		//		{
		//			note: "rego.v1 not imported, with feature",
		//			policy: `package test
		//a := 1`,
		//			features: []string{"rego_v1_import"},
		//		},
	...
	for _, tc := range tests {
		t.Run(tc.note, func(t *testing.T) {
			...
			test.WithTempFS(files, func(path string) {
				params := newEvalCommandParams()
				params.capabilities.C = ast.CapabilitiesForThisVersion()
				params.capabilities.C.Features = tc.features
				_ = params.dataPaths.Set(filepath.Join(path, fileName))

				var buf bytes.Buffer
				_, err := eval([]string{"data"}, params, &buf)
				...
				if tc.expectedError != "" {
					...
					if msg := output.Errors[0].Message; msg != tc.expectedError {
						t.Errorf("expected message '%v', got '%v'", tc.expectedError, msg)
					}
				} else if err != nil {

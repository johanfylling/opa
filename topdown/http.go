...
func verifyHost(bctx BuiltinContext, unverifiedUrl string) error {
	if bctx.AllowNet == nil {
		return nil
	}
	...
	for _, allowed := range bctx.AllowNet {
		if allowed == host {
			return nil
		}
	}

	return fmt.Errorf("remote host not allowed for: %s", unverifiedUrl)
}

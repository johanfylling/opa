...
func TestLogBuffer(t *testing.T) {
	...
	dropped, _ := buffer.Push(make([]byte, 20), true)
	if dropped != 0 {
	...
	dropped, _ = buffer.Push(bytes.Repeat([]byte(`1`), 10), true)
	if dropped != 0 {
		t.Fatal("Expected dropped to be zero")
	}

	dropped, _ = buffer.Push(bytes.Repeat([]byte(`2`), 10), true)
	if dropped != 0 {
		t.Fatal("Expected dropped to be zero")
	}

	dropped, _ = buffer.Push(bytes.Repeat([]byte(`3`), 10), true)
	if dropped != 1 {
